/*
		Normalized version 1.2, single link/ with Euclidean distance function.
		Sevval Boylu- 7.08.2017
*/
...
int divCluster(int **RTT_Predict, int *clusters)
{
	...
	double max_distance=0; int index1=0, index2=0;
	if(count_clusters < 5)
	{
		for(int i=0; i<1000; i++)
		{
			for(int j=i+1; j<1000; j++)
			{
				if(clusters[i]==clusters[j])
				{
					...
					if(sq_distance>max_distance)
					{
						max_distance=sq_distance;
						index1=i; index2=j;
					}
				}
			}
		...
		//NOW WE HAVE THE FARTHEST TWO POINTS. ITS TIME TO SPLIT THE CLUSTER INTO TWO.
		...
		for(int m=0; m<1000; m++)
		{
			...
			if(clusters[m]== clusters[index1] && clusters[m]== clusters[index2])
			{
				...
				for(int k=0; k<10;k++)
				{
					dist1+= ((RTT_Predict[m][k]-RTT_Predict[index1][k])*(RTT_Predict[m][k]-RTT_Predict[index1][k]));
					dist2+= ((RTT_Predict[m][k]-RTT_Predict[index2][k])*(RTT_Predict[m][k]-RTT_Predict[index2][k]));
				}
