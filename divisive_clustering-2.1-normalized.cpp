/*
		Normalized version 2.1, complete linkage / euclidean distance formula.
		Sevval Boylu- 26.09.2017
*/
...
//clustering function
/*I feel like this algorithm needs a bit pre-explaination,
	I first look at every point and its distance to others. By this I find the nearest point to that point, which I call
	these two points "consecutive" points. Then I find the farthest consecutive points and assign them as index1 and index2,
	which are the indexes that are used to split the cluster by assigning every point in cluster to either one of these indexes' clusters.
	I used complete linkage by this way, kinda reversed version of it since this is divisive clustering not agglomerative.
*/
	...
	if(count_clusters < 5)
	{
		for(int i=0; i<1000; i++)
		{
			for(int j=i+1; j<1000; j++)
			{
				if(clusters[i]==clusters[j])
				{
					...
					for(int k=0; k<10;k++)
					{
						distance+= ((RTT_Predict[i][k]-RTT_Predict[j][k])*(RTT_Predict[i][k]-RTT_Predict[j][k]));
					}
					double sq_distance=sqrt(distance); 
		...
		for(int m=0; m<1000; m++)
		{
			...
			if(clusters[m]== clusters[index1] && clusters[m]== clusters[index2])
			{
				...
				for(int k=0; k<10;k++)
				{
					dist1+= ((RTT_Predict[m][k]-RTT_Predict[m_index1][k])*(RTT_Predict[m][k]-RTT_Predict[m_index1][k]));
					dist2+= ((RTT_Predict[m][k]-RTT_Predict[m_index2][k])*(RTT_Predict[m][k]-RTT_Predict[m_index2][k]));
				}

				long double sqdist1, sqdist2;
				sqdist1= sqrt(dist1);
				sqdist2= sqrt(dist2);
