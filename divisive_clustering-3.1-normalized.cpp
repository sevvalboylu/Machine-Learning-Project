/*
		DROPS SO MUCH DOWN SOMETIMES, SOMETHING MAY BE WRONG WITH THE ALGORITHM

		Normalized version 3.1, average link/ with Euclidean distance function.
		Sevval Boylu- 27.09.2017
*/
...
int divCluster(int **RTT_Predict, int *clusters)
{
	...
	if(count_clusters < 5)
	{
			...
			for(int i=0; i<1000; i++)
			{
				for(int j=i+1; j<1000; j++)
				{
					if(clusters[i]==clusters[j]&& clusters[i]==k)
					{
						...
						for(int m=0; m<10; m++)
						{
							distance+= ((RTT_Predict[i][m]-RTT_Predict[j][m])*(RTT_Predict[i][m]-RTT_Predict[j][m]));
						}
		...
		//we have the averages, now we find the one with the largest average and split this cluster
		int index;
		double max=0;
		...
		// zero point will be the point which is closest to the origin
		...
		for(int i=0; i<1000; i++)
		{
			if(clusters[i]==index)
			{
				...
				for(int k=0; k<10; k++)
				{
					dist+= (RTT_Predict[i][k])*(RTT_Predict[i][k]);
				}
		...
		for(int i=0; i<1000; i++)
		{
			if(clusters[i]==index)
			{
				...
				for(int k=0; k<5; k++)
				{ 
					distance+= ((RTT_Predict[i][k]-RTT_Predict[min_index][k])*(RTT_Predict[i][k]-RTT_Predict[min_index][k]));
				}
				double sq_distance=sqrt(distance);
				if(sq_distance>average[index])
				{
