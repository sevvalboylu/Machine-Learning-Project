/*
		Normalized version 1.1, single link / with the original distance function.
	...
	if(count_clusters < 5)
	{
		...
		for(int m=0; m<1000; m++)
		{
			...
			if(clusters[m]== clusters[index1] && clusters[m]== clusters[index2])
			{
				...
				//Finding distances of all points and if one point is closer to one cluster than the other,
				//we add the index of that point to that cluster.
				for(int k=0; k<10 ; k++)
				{
				...
				dist1=diff1/sum1;
				dist2=diff2/sum2;
				//If the point is near to the second point, we change its label, otherwise we dont change its label
				//this way we split the cluster into two.
				if(dist1<dist2)
				{
